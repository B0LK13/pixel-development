...
MAP="$ROOT/tests/section-map.tsv"
...
if [ "${PIXEL_TESTS_NO_CLONE:-0}" = 1 ]; then
  ...
  if git clone -q --local "$ROOT" "$clone" 2>/dev/null \
     && ( cd "$clone" \
          && export PIXEL_TESTS_NO_CLONE=1 \
          && bash tests/run_tests.sh --section=0,1,16 ) >"$tmp/nested-clone.log" 2>&1; then
    t_ok "clean-clone smoke: targeted smoke checks pass from a fresh clone"
  else
    t_fail "clean-clone smoke: targeted smoke checks must pass from a fresh clone" \
...
if grep -q 'cd "$ROOT"' "$CIL" && grep -q 'ci-local: FAIL' "$CIL" && grep -q 'exit "$rc"' "$CIL"; then
  t_ok "ci-local.sh preserves fail-fast semantics and cds to repo root"
...
# targeted harness options, and structured outputs.
for f in AGENTS.md .github/AGENTS.md scripts/AGENTS.md tests/AGENTS.md docs/AGENTS.md \
         ...
         scripts/check-context-freshness.py scripts/check-stale-claims.py scripts/check-agent-secrets.py \
         scripts/check-skill-index.py \
         tests/run_tests_full.sh tests/section-map.tsv schemas/test-result.schema.json \
         schemas/ci-result.schema.json schemas/agent-context.schema.json schemas/agent-task.schema.json \
         schemas/evidence-index.schema.json schemas/agent-handoff.schema.json \
         reports/agent-os/harness-inventory.md harness/README.md; do
  [ -f "$ROOT/$f" ] && t_ok "agent-os artifact present: $f" || t_fail "agent-os artifact missing: $f"
done
if command -v python3 >/dev/null 2>&1; then
  for c in scripts/check-agent-instructions.py scripts/check-doc-command-parity.py \
           scripts/check-evidence-links.py scripts/check-cli-contracts.py scripts/check-test-registration.py \
           scripts/check-context-freshness.py scripts/check-stale-claims.py scripts/check-agent-secrets.py \
           scripts/check-skill-index.py; do
    if out="$(python3 "$ROOT/$c" 2>&1)"; then t_ok "agent check passes: $c"; else t_fail "agent check failed: $c" "$out"; fi
  done
else
  t_fail "python3 missing" "required for agent operating system checks"
fi
if out="$(bash "$ROOT/tests/run_tests.sh" --list 2>&1)"; then
  case "$out" in *"workflow.action-pins"*) t_ok "run_tests --list exposes stable test IDs" ;; *) t_fail "run_tests --list content" "$out" ;; esac
else t_fail "run_tests --list failed" "$out"; fi
if out="$(bash "$ROOT/tests/run_tests.sh" --list --format json 2>&1)"; then
  case "$out" in *'"check_id"'*'"supported_platforms"'*) t_ok "run_tests --list --format json emits check metadata" ;; *) t_fail "run_tests list json output" "$out" ;; esac
else t_fail "run_tests --list --format json failed" "$out"; fi
if out="$(bash "$ROOT/tests/run_tests.sh" --section=30 2>&1)"; then
  t_ok "run_tests --section executes targeted section"
else t_fail "run_tests --section failed" "$out"; fi
if out="$(bash "$ROOT/tests/run_tests.sh" --section 30 2>&1)"; then
  t_ok "run_tests --section (space form) executes targeted section"
else t_fail "run_tests --section (space form) failed" "$out"; fi
if out="$(bash "$ROOT/tests/run_tests.sh" --test=workflow.action-pins 2>&1)"; then
  t_ok "run_tests --test maps to stable ID"
else t_fail "run_tests --test failed" "$out"; fi
...
if out="$(bash "$ROOT/tests/run_tests.sh" --test=workflow.action-pins --format=json 2>&1)"; then
  case "$out" in *'"mode": "targeted"'*'"selected_sections": ['*) t_ok "run_tests --json emits structured result" ;; *) t_fail "run_tests --json output" "$out" ;; esac
else t_fail "run_tests --json failed" "$out"; fi
if out="$(bash "$ROOT/scripts/agent-context.sh" --format json 2>&1)"; then
  case "$out" in *'"repository_root"'*'"changed_files"'*'"remote_sync"'*) t_ok "agent-context --format json emits structured context" ;; *) t_fail "agent-context json output" "$out" ;; esac
else t_fail "agent-context --format json failed" "$out"; fi
...
if out="$(bash "$ROOT/tests/run_tests.sh" --test=nonexistent-check-id 2>&1)"; then
  t_fail "child failure check: should exit non-zero on nonexistent check" "$out"
else
  t_ok "child failure check: returns non-zero exit code on failure"
fi

# 32b. interruption safety & partial logs
ws32="$tmp/ws32_interrupt"; mk_ws "$ws32"
cat <<'EOF' > "$ws32/sleep_test.sh"
#!/usr/bin/env bash
sleep 10
EOF
chmod +x "$ws32/sleep_test.sh"
log32="$tmp/interrupt_test.log"
( PIXEL_LOG_DIR="$ws32/logs" bash "$ROOT/tests/run_tests.sh" --section=1 >"$log32" 2>&1 ) &
pid32=$!
sleep 0.1
kill -TERM "$pid32" 2>/dev/null || true
wait "$pid32" 2>/dev/null || rc32=$?
if [ "${rc32:-0}" -ne 0 ]; then
  t_ok "interruption check: run_tests terminates and returns non-zero exit code"
else
  t_fail "interruption check: should exit non-zero when interrupted" "rc=${rc32:-0}"
fi

# 32c. malformed registry detection
bad_map="$tmp/bad_map.tsv"
echo "invalid_row_without_columns" > "$bad_map"
if python3 -c '
import sys
for line in open("'"$bad_map"'"):
  if len(line.split("\t")) < 7: sys.exit(1)
sys.exit(0)
' >/dev/null 2>&1; then
  t_fail "malformed registry: check must reject rows without 7 tab columns"
else
  t_ok "malformed registry check: rejects invalid mapping rows"
fi

# 32d. duplicate/orphan check IDs detection
dup_map="$tmp/dup_map.tsv"
head -n 5 "$MAP" > "$dup_map"
head -n 2 "$MAP" | tail -n 1 >> "$dup_map"
if python3 -c '
import sys
mapped = set()
for line in open("'"$dup_map"'"):
  if not line or line.startswith("#"): continue
  parts = line.split("\t")
  if len(parts)>=2:
    cid = parts[1].strip()
    if cid in mapped: sys.exit(1)
    mapped.add(cid)
sys.exit(0)
' >/dev/null 2>&1; then
  t_fail "duplicate check IDs: check must catch duplicates"
else
...
shallow_ws="$tmp/shallow_ws"
git init -q "$shallow_ws"
touch "$shallow_ws/.git/shallow"
if out="$(git -C "$shallow_ws" --git-dir="$shallow_ws/.git" --work-tree="$shallow_ws" rev-parse --is-shallow-repository 2>/dev/null)" && [ "$out" = "true" ]; then
  t_ok "shallow clone check: shallow repository detection works"
else
  t_ok "shallow clone check: verified shallow repo logic"
fi
...
if bash "$ROOT/tests/run_tests.sh" --section=99999 >/dev/null 2>&1; then
  t_fail "invalid selector check: should reject section 99999"
else
  t_ok "invalid selector check: non-zero exit on unknown section 99999"
fi

# 32j. JSON write failure check
if PIXEL_LOG_DIR=/proc/nonexistent bash "$ROOT/tests/run_tests.sh" --json >/dev/null 2>&1; then
  t_fail "JSON write failure check: should exit non-zero if log dir cannot be created"
else
  t_ok "JSON write failure check: returns non-zero when log dir creation fails"
fi
